...
class Table {
  ...
  static async getAll() {
    const result = await this.pool.query(
      `select * from ${schemaPrefix}${this.name}`
    );
    return result.rows;
  }
  ...
  static async getWhere(whereClause, params) {
    const result = await this.pool.query(
      `select * from ${schemaPrefix}${this.name} where ${whereClause}`,
      params
    );
    return result.rows;
  }
  ...
  static async add(entryObject) {
    ...
    const result = await this.pool.query(
      `insert into ${schemaPrefix}${this.name}(${dynamicFieldSet}) values (${dynamicParam})`,
      dynamicValues
    );
  ...
  static async update(updatedObject) {
    ...
    dynamicUpdateSet = dynamicUpdateSet.substring(
      0,
      dynamicUpdateSet.length - 1
    );

    for (let primaryKey in primaryKeyValueObj) {
      dynamicWhereClause = dynamicWhereClause.concat(
        `${primaryKey}=$${counter} and `
      );
      params.push(primaryKeyValueObj[primaryKey]);
      counter += 1;
    }

    dynamicWhereClause = dynamicWhereClause.substring(
      0,
      dynamicWhereClause.length - 4
    );

    const result = await this.pool.query(
      `update ${schemaPrefix}${this.name} set ${dynamicUpdateSet} where ${dynamicWhereClause}`,
      params
    );
    return result.rows;
  }
  ...
  static async delete(deleteObject) {
    ...
    const result = await this.pool.query(
      `delete from ${schemaPrefix}${this.name} where ${dynamicWhereClause}`,
      params
    );
