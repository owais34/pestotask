...
class Table {
  ...
  static async getWhere(whereClause){
    const result = await pool.query(`select * from ${this.name} where ${whereClause}`)
    return result.rows
  }
  ...
  static async add(entryObject){
    ...
    const result = await pool.query(`insert into ${this.name}(${dynamicFieldSet}) values (${dynamicParam})`,dynamicParam)
  ...
  static async update(updatedObject) {
    ...
    const result = await pool.query(`update ${this.name} set ${dynamicUpdateSet} where ${dynamicWhereClause}`,params)
    return result.rows
  }
  ...
  static async delete(deleteObject){
    ...
    dynamicWhereClause = dynamicWhereClause.substring(0,dynamicWhereClause.length -4);
    const result = await pool.query(`delete from ${this.name} where ${dynamicWhereClause}`,params)
